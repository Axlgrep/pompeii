...
void UnlinkTest::TearDownTestCase() {
  redisFree(ctx);
}

void UnlinkTest::SetUp() {
...
TEST_F(UnlinkTest, UNLINK_CASE_1) {
  res = (redisReply*)redisCommand(ctx, "UNLINK");
  ASSERT_EQ(REDIS_REPLY_ERROR, res->type);
  ASSERT_STREQ("ERR wrong number of arguments for 'unlink' command", res->str);
  freeReplyObject(res);
}

TEST_F(UnlinkTest, UNLINK_CASE_2) {
  res = (redisReply*)redisCommand(ctx, "UNLINK 06S 6ZJ");
  ASSERT_EQ(REDIS_REPLY_INTEGER, res->type);
  ASSERT_EQ(0, res->integer);
  freeReplyObject(res);
}

TEST_F(UnlinkTest, UNLINK_CASE_3) {
  ...
  res = (redisReply*)redisCommand(ctx, "MGET 06S 6ZJ");
  ASSERT_EQ(REDIS_REPLY_ARRAY, res->type);
  ASSERT_EQ(REDIS_REPLY_NIL, res->element[0]->type);
  ASSERT_EQ(REDIS_REPLY_NIL, res->element[1]->type);
  freeReplyObject(res);
}

TEST_F(UnlinkTest, UNLINK_CASE_4) {
